...
@section('content')
    <div class="service-list-container">
        <div class="card">
            ...
            {{-- Controller mengirim: $tour, $services, $transportations, $tourItems, $statuses --}}
            <form action="{{ route('tour.customer.update', $tour->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="card-body">
                    ...
                    {{-- BARIS 1: SERVICE, NAMA TOUR, TRANSPORTASI --}}
                    <div class="row">
                        <div class="col-md-4">
                                ...
                                <select id="service_id" name="service_id"
                                    ...
                                    @foreach ($services as $service)
                                        <option value="{{ $service->id }}"
                                            {{ old('service_id', $tour->service_id) == $service->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="tour_id" class="form-label">Nama Tour</label>
                                <select id="tour_id" name="tour_id"
                                    class="form-select @error('tour_id') is-invalid @enderror" disabled>
                                    <option value="">Pilih Tour</option>
                                    @foreach ($tourItems as $item)
                                        <option value="{{ $item->id }}"
                                            {{ old('tour_id', $tour->tour_id) == $item->id ? 'selected' : '' }}>
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                <label for="transportation_id" class="form-label">Transportasi</label>
                                <select id="transportation_id" name="transportation_id"
                                    ...
                                    @foreach ($transportations as $transport)
                                        <option value="{{ $transport->id }}"
                                            {{ old('transportation_id', $tour->transportation_id) == $transport->id ? 'selected' : '' }}>
                                            {{ $transport->nama }}
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    {{-- BARIS 2: TANGGAL, STATUS, SUPPLIER --}}
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="tanggal_keberangkatan" class="form-label">Tanggal Keberangkatan Tour</label>
                                <input type="date" id="tanggal_keberangkatan" name="tanggal_keberangkatan"
                                    class="form-control @error('tanggal_keberangkatan') is-invalid @enderror" disabled
                                    value="{{ old('tanggal_keberangkatan', \Carbon\Carbon::parse($tour->tanggal_keberangkatan)->format('Y-m-d')) }}">
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="text" id="supplier" name="supplier"
                                    ...
                                    value="{{ old('supplier', $tour->supplier) }}">
                                @error('supplier')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="number" step="0.01" id="harga_dasar" name="harga_dasar"
                                    ...
                                    value="{{ old('harga_dasar', $tour->harga_dasar) }}">
                                @error('harga_dasar')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="number" step="0.01" id="harga_jual" name="harga_jual"
                                    ...
                                    value="{{ old('harga_jual', $tour->harga_jual) }}">
                                @error('harga_jual')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
