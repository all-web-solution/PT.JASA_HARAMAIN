...
@section('title', 'Edit Order Transportasi')
    ...
    <div class="service-list-container">
        <div class="card">
            <div class="card-header">
                <h5 class="card-title">
                    <i class="bi bi-truck"></i> Edit Order Transportasi
                </h5>
                {{-- Tombol Batal kembali ke halaman detail --}}
                <a href="{{ route('transportation.car.detail.customer', $item->id) }}" class="btn btn-secondary">
                    <i class="bi bi-arrow-left"></i> Kembali
                </a>
            </div>

            {{-- Controller mengirim: $item, $services, $transportations, $routes, $statuses --}}
            <form action="{{ route('transportasi.customer.update', $item->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="card-body">
                    ...
                    {{-- BARIS 1: SERVICE & NAMA TRANSPORTASI --}}
                    <div class="row">
                        <div class="col-md-6">
                                ...
                                <select id="service_id" name="service_id"
                                    ...
                                    @foreach ($services as $service)
                                        <option value="{{ $service->id }}"
                                            {{ old('service_id', $item->service_id) == $service->id ? 'selected' : '' }}>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label for="transportation_id" class="form-label">Nama Transportasi</label>
                                <select id="transportation_id" name="transportation_id"
                                    class="form-select @error('transportation_id') is-invalid @enderror" disabled>
                                    <option value="">Jenis Transportasi</option>
                                    @foreach ($transportations as $transport)
                                        <option value="{{ $transport->id }}"
                                            {{ old('transportation_id', $item->transportation_id) == $transport->id ? 'selected' : '' }}>
                                            {{ $transport->nama }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="date" id="dari_tanggal" name="dari_tanggal"
                                    ...
                                    value="{{ old('dari_tanggal', \Carbon\Carbon::parse($item->dari_tanggal)->format('Y-m-d')) }}"
                                    disabled>
                                @error('dari_tanggal')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="date" id="sampai_tanggal" name="sampai_tanggal"
                                    ...
                                    value="{{ old('sampai_tanggal', \Carbon\Carbon::parse($item->sampai_tanggal)->format('Y-m-d')) }}"
                                    disabled>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select id="route_id" name="route_id"
                                    class="form-select @error('route_id') is-invalid @enderror" disabled>
                                    <option value="">Rute</option>
                                    @foreach ($routes as $route)
                                        <option value="{{ $route->id }}"
                                            {{ old('route_id', $item->route_id) == $route->id ? 'selected' : '' }}>
                                            {{ $route->route }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="col-md-4">
                            <div class="form-group">
                                ...
                                <input type="number" step="0.01" id="harga_dasar" name="harga_dasar"
                                    ...
                                    value="{{ old('harga_dasar', $item->harga_dasar) }}">
                                @error('harga_dasar')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
